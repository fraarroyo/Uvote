...
def run_migrations():
    ...
    with app.app_context():
        try:
            # First, create the party_list table
            ...
            # Add college_id column to candidate table if it doesn't exist
            try:
                db.session.execute(text('''
                    ALTER TABLE candidate ADD COLUMN college_id INTEGER
                    REFERENCES college (id)
                '''))
            except Exception as e:
                print("Column college_id might already exist:", str(e))
